...
export function ChatInput({ onSend, onAbort, disabled }: ChatInputProps) {
  ...
  return (
    <div
      className={`border-t border-[#E2E8F0] bg-white px-4 py-3 ${isDragging ? "ring-2 ring-inset ring-[#2563EB]/20" : ""}`}
      ...
      <div className="flex items-end gap-2">
        ...
        <div className="flex-1 relative">
          <textarea
            ...
            disabled={disabled}
            placeholder={isDragging ? "拖放文件到此处..." : "输入消息..."}
            rows={1}
        ...
        {disabled && onAbort ? (
          <button
            ...
            className="shrink-0 w-9 h-9 rounded-lg bg-[#EF4444] hover:bg-[#DC2626] flex items-center justify-center transition-colors duration-150 cursor-pointer"
            aria-label="中断"
          >
